...
import KartelaModal from './kartelamodal';
...
export default function Row(props) {
    ...
    const [showKartelaModal, setShowKartelaModal] = useState(false)
    ...
    const handleClickKartela = (e) => {
        const information = e.split('-')
        setDataModal(information[0])
        setDataModalName(information[1])
        setShowKartelaModal(true)
    }
        ...
        <Mui_table.Body>
            {props.visibleRows.map((data, index) => (
                <Mui_table.Row >
                    ...
                    <Mui_table.Cell align='center' className='border border-1 border-black px-0 fs-5'>
                        <button className='btn btn-success' id={`${data.CSTMCODE}`+'-'+`${data.CSTMNAME}`} onClick={(e) => {
                                handleClickKartela(e.target.id)
                            }}>Kartela
                        </button>
                        {showKartelaModal &&  <KartelaModal 
                            setShowKartelaModal={setShowKartelaModal}
                            dataModal={dataModal}
                            dataModalName={dataModalName}
                        />}
                    </Mui_table.Cell>
